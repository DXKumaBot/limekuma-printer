...
public class LxnsDeveloperClient(string token) : LxnsDataClient("Authorization", token)
{
    ...
    public async Task UploadRecordsAsync(long friendCode, IEnumerable<Record> records,
        CancellationToken cancellationToken = default) => await PostAsync($"/api/v0/maimai/player/{friendCode}/scores",
        new { records }, cancellationToken);
    ...
    public async Task<List<SimpleRecord>>
        GetAllRecordsAsync(long friendCode, CancellationToken cancellationToken = default) =>
    ...
    public async Task<Dictionary<string, int>>
        GetHeatmapAsync(long friendCode, CancellationToken cancellationToken = default) =>
    ...
    public async Task
        UploadFromHtmlAsync(long friendCode, string html, CancellationToken cancellationToken = default) =>
